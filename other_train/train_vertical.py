...
if __name__ == "__main__":
  ...
  flags.DEFINE_string("label_loss", "CrossEntropyLossWithVertical",
    ...
    with tf.device(device_string % i):
      with (tf.variable_scope(("tower"), reuse=True if i > 0 else None)):
        with (slim.arg_scope([slim.model_variable, slim.variable], device="/cpu:0" if num_gpus!=1 else "/gpu:0")):
          ...
          predictions_vertical = result["predictions_vertical"]

          if "loss" in result.keys():
            label_loss = result["loss"]
          else:
            label_loss = label_loss_fn.calculate_loss(predictions, predictions_vertical, mapping_vertical, tower_labels[i])
...
class Trainer(object):
  ...
  def __init__(self, cluster, task, train_dir, model, reader, model_exporter,
               ...
               export_model_steps=1000, mapping_vertical=None):
    """"Creates a Trainer.
    ...
    self.mapping_vertical = mapping_vertical
...
#                           task_as_string(self.task))
    ...
    with tf.Graph().as_default() as graph:
      ...
      with tf.device(device_fn):
        if not meta_filename:
          saver = self.build_model(self.model, self.reader, self.mapping_vertical)
  ...
  def build_model(self, model, reader, mapping_vertical=None):
    ...
    build_graph(reader=reader,
                 ...
                 mapping_vertical=mapping_vertical)
...
def main(unused_argv):
  ...
  if not cluster or task.type == "master" or task.type == "worker":
    ...
    file_dir = '/home/weimin/yt8m/code/youtube-8m/index_to_vertical_1_mapping.npz'

    with open(file_dir, 'rb') as f:
        mapping_vertical = np.load(f)
        mapping_vertical = mapping_vertical.astype(np.float32)

    Trainer(cluster, task, FLAGS.train_dir, model, reader, model_exporter,
            FLAGS.log_device_placement, FLAGS.max_steps,
            FLAGS.export_model_steps, mapping_vertical).run(start_new_model=FLAGS.start_new_model)
