...
class Trainer(object):
  ...
  def __init__(self, cluster, task, train_dir, model, embeddings, reader, model_exporter,
...
#                           task_as_string(self.task))
    ...
    with tf.Graph().as_default() as graph:
      ...
      with tf.device(device_fn):
        if not meta_filename:
          saver = self.build_model(self.model, self.reader, self.embeddings)
  ...
  def build_model(self, model, reader, embeddings):
...
def main(unused_argv):
  ...
  if not cluster or task.type == "master" or task.type == "worker":
    ...
    df_vec = pd.read_csv('/home/weimin/yt8m/code/youtube-8m/vec_ready_to_use_v1.0.csv')
    mat = df_vec.iloc[:, :300].values
    mat = np.transpose(mat, [1,0])

    Trainer(cluster, task, FLAGS.train_dir, model, mat, reader, model_exporter,
