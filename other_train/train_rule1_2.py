...
def build_graph(reader,
  ...
  ############# apply rules here 
  mat1_tf = tf.convert_to_tensor(mat1, tf.float32)
  mat2_tf = tf.convert_to_tensor(mat2, tf.float32)

  labels_batch = tf.cast(labels_batch, tf.float32)
  labels_added = tf.matmul(labels_batch, mat1_tf)
  labels_added = tf.matmul(labels_added, mat2_tf)

  labels_batch = labels_batch + labels_added
  labels_batch = tf.cast(labels_batch, tf.bool)
  ############# apply rules here 
...
class Trainer(object):
  ...
  def __init__(self, cluster, task, train_dir, model, reader, model_exporter,
               ...
               export_model_steps=1000, mat1 = None, mat2 = None):
    """"Creates a Trainer.
...
#                           task_as_string(self.task))
    ...
    with tf.Graph().as_default() as graph:
      ...
      with tf.device(device_fn):
        if not meta_filename:
          saver = self.build_model(self.model, self.reader, self.mat1, self.mat2)
    ...
    with sv.managed_session(target, config=self.config) as sess:
      try:
        ...
        while (not sv.should_stop()) and (not self.max_steps_reached):
          ...
          #if not np.all(labels_val == labels_original_val):
          #  for i in range(labels_val.shape[0]):
          #    if not np.all(labels_val[i] == labels_original_val[i]):
          #      print(np.where(labels_val[i])[0], " ---- ", np.where(labels_original_val[i])[0])
          #      break
  ...
  def build_model(self, model, reader, mat1, mat2):
...
def main(unused_argv):
  ...
  if not cluster or task.type == "master" or task.type == "worker":
    ...
    Trainer(cluster, task, FLAGS.train_dir, model, reader, model_exporter,
            ...
            FLAGS.export_model_steps, rule1_mat, rule2_mat).run(start_new_model=FLAGS.start_new_model)
