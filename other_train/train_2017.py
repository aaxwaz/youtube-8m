    ...
    with tf.device(device_string % i):
      with (tf.variable_scope(("tower"), reuse=True if i > 0 else None)):
        with (slim.arg_scope([slim.model_variable, slim.variable], device="/cpu:0" if num_gpus!=1 else "/gpu:0")):
          ...
          val_bool = tf.equal(tower_labels[i][:, -1], False)

          predictions = tf.boolean_mask(predictions, val_bool)
          tower_labels_temp = tf.boolean_mask(tower_labels[i], val_bool)
          tower_labels_temp = tower_labels_temp[:, :-1]

          print("\n\n{}\n\n".format(tower_labels_temp))
          print("\n\n{}\n\n".format(predictions))
          label_loss = label_loss_fn.calculate_loss(predictions, tower_labels_temp)
...
def get_reader(mapping_2017):
  ...
  if FLAGS.frame_features:
    reader = readers.YT8MFrameFeatureReader(
        feature_names=feature_names, feature_sizes=feature_sizes, num_classes=4716, mapping_mat=mapping_2017)
  else:
...
def main(unused_argv):
  ...
  if not cluster or task.type == "master" or task.type == "worker":
    ...
    reader = get_reader(mapping_2017)
