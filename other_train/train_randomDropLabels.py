...
def build_graph(reader,
  ...
  ## reduction sampling
  sampling_probs = tf.random_uniform([batch_size,3862] , 0, 1)
  labels_batch_sampled = tf.multiply(tf.cast(sampling_probs < 0.85, tf.float32), labels_batch)

  ## for rows that have become zero label, random choose one label to assign to them
  all_zero_rows = tf.cast(tf.reduce_sum(labels_batch_sampled, 1, keepdims = True) < 1, tf.float32) # None x 1
  #labels_batch_back = tf.multiply(all_zero_rows, labels_batch)
  sample_one_only = tf.multiply(labels_batch, sampling_probs)                 # None x 3862
  sample_one_only_max = tf.reduce_max(sample_one_only, 1, keepdims = True)    # None x 1 
  labels_batch_back = tf.multiply(tf.cast(sample_one_only >= sample_one_only_max, tf.float32), all_zero_rows) 

  labels_batch_sampled = labels_batch_sampled + labels_batch_back

  tower_inputs = tf.split(model_input, num_towers)
  tower_labels = tf.split(labels_batch_sampled, num_towers)
  ...
  tf.add_to_collection("labels", tf.cast(labels_batch_sampled, tf.float32))
  tf.add_to_collection("original_labels", tf.cast(labels_batch, tf.float32))
  tf.add_to_collection("train_op", train_op)
  tf.add_to_collection("sampling_probs", sampling_probs)
