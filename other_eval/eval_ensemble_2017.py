...
import readers_2017 as readers
...
if __name__ == "__main__":
  ...
  flags.DEFINE_string(
      "load_this_checkpoint", "", 
      "If none, will load latest checkpoint")
  flags.DEFINE_boolean("save_preds", True, "Whether to save predictions.")
  flags.DEFINE_integer("total_ens_times", 1,
                       "How many rounds of ensemble to run.")
  #flags.DEFINE_integer("model_ckpt_for_evaluation", )
...
def evaluation_loop(video_id_batch, prediction_batch, label_batch,
                    summary_op, saver, summary_writer, evl_metrics,
                    last_global_step_val, total_ens_times = 1):
  ...
  for e in range(total_ens_times):
    ...
    with tf.Session() as sess:
      ...
      fetches = [video_id_batch, prediction_batch, label_batch, summary_op]
      ...
      try:
        ...
        while not coord.should_stop():
          ...
          fake_loss = np.array([0.] * labels_val.shape[0]).reshape(-1,1)
    ...
    if FLAGS.save_preds:
      temp_df.to_csv(os.path.join(FLAGS.train_dir, 'eval_predictions.csv'), index = True)
    ...
    if len(final_df) == 1:
      ...
      final_preds = np.concatenate([np.expand_dims(temp_df.ix[final_index]['preds'].values, 2) for temp_df in final_df], axis = 2)
      final_preds = hmean(final_preds, axis = 2)

    fake_loss_zeros = np.zeros((final_df[0].shape[0], 1))

    evl_metrics.clear()
    iteration_info_dict = evl_metrics.accumulate(final_preds,
                                                 final_labels, fake_loss_zeros)
  ...
  with tf.Graph().as_default():
    ...
    while True:
      last_global_step_val = evaluation_loop(video_id_batch, prediction_batch,
                                             label_batch, summary_op,
                                             saver, summary_writer, evl_metrics,
                                             last_global_step_val, FLAGS.total_ens_times)
      if FLAGS.run_once:
        break
