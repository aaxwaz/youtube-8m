...
class MultiscaleCnnLstmModel(models.BaseModel):
  ...
  def rnn(self, model_input, lstm_size, num_frames, 
    ...
    with tf.variable_scope("RNN-"+sub_scope):
      outputs, state = tf.nn.dynamic_rnn(cell, model_input,
                                         ...
                                         swap_memory=True,
                                         dtype=tf.float32)
      n = outputs.get_shape().as_list()[1]
      num_frames_half = tf.cast((num_frames-1)/2, tf.int32)
      mask = tf.one_hot(num_frames_half, depth=n, dtype=tf.bool, on_value=True, off_value=False)
      half_frame_output = tf.boolean_mask(outputs, mask)
    # return final memory
    return tf.concat([state.c, half_frame_output], 1)
  ...
  def create_model(self, model_input, vocab_size, num_frames, is_training=True,
    ...
    num_layers = 3
    lstm_size = 800
    activation_proj_dim = int(lstm_size*1.28)
    pool_size=2
    ...
    for layer in range(num_layers):
      ...
      #lstm_memory = slim.batch_norm(
      #  lstm_memory,
      #  center=True,
      #  scale=True,
      #  is_training=self.is_training,
      #  scope="lstm_memory_bn_layer_"+str(layer))
      ...
      cnn_max_frames = int(cnn_max_frames)
      max_pooled_cnn_output = tf.reduce_max(
          tf.reshape(
              cnn_output_tanh[:, :int(cnn_max_frames*2), :], 
