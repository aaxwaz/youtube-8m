...
class LightVLAD():
    ...
    def forward(self,reshaped_input):
        ...
        cluster_weights_vlad = tf.get_variable("cluster_weights_vlad",
              [self.feature_size, self.cluster_size],
              initializer = tf.random_normal_initializer(stddev=1 / math.sqrt(self.feature_size)))

        activation = tf.matmul(reshaped_input, cluster_weights) # None (None * max_frames) x cluster_size 
        activation_vlad = tf.matmul(reshaped_input, cluster_weights_vlad) # None (None * max_frames) x cluster_size 
        ...
        activation = slim.batch_norm(
            ...
            scope="cluster_bn")
        activation_vlad = slim.batch_norm(
            activation_vlad,
            center=True,
            scale=True,
            is_training=self.is_training,
        ...
        activation_vlad = tf.reshape(activation_vlad, [-1, self.max_frames, self.cluster_size]) # None x max_frames x cluster_size

        ### only to vlad ###
        a_sum = tf.reduce_sum(activation_vlad,-2,keep_dims=True) # None x 1 x cluster_size
        cluster_weights2 = tf.get_variable("cluster_weights_vlad_2",
        ...
        activation_vlad = tf.transpose(activation_vlad,perm=[0,2,1]) # None x cluster_size x max_frame

        reshaped_input = tf.reshape(reshaped_input,[-1,self.max_frames,self.feature_size]) # None x max_frame x feature_size

        ### only to light ###
        lightvlad = tf.matmul(activation,reshaped_input)        # None x cluster_size x feature_size
        lightvlad = tf.transpose(lightvlad,perm=[0,2,1]) # None x feature_size x cluster_size 
        ### only to light ###

        ### only to vlad ###
        vlad = tf.matmul(activation_vlad,reshaped_input) # None x cluster_size x feature_size
        ...
        vlad_final = tf.reshape(vlad_final,[-1,self.cluster_size*self.feature_size])
        vlad_final = tf.nn.l2_normalize(vlad_final,1)
